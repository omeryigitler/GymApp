import { Activity, ArrowLeft, Dumbbell, Flame, TrendingUp } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col bg-[radial-gradient(circle_at_top,_rgba(37,99,235,0.18),_transparent_36%),#020617] h-full">
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar px-5 py-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-gradient-to-br from-slate-900/95 to-slate-950 border border-white/10 rounded-[24px] p-4">
            <Activity className="text-blue-400 mb-3" size={20} />
            <p className="text-2xl font-black text-white">{workouts.length}</p>
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">Antrenman</p>
          </div>
          <div className="bg-gradient-to-br from-slate-900/95 to-slate-950 border border-white/10 rounded-[24px] p-4">
            <Dumbbell className="text-emerald-400 mb-3" size={20} />
            <p className="text-2xl font-black text-white">{completedSets}</p>
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">Set</p>
          </div>
          <div className="bg-gradient-to-br from-slate-900/95 to-slate-950 border border-white/10 rounded-[24px] p-4">
            <Flame className="text-orange-400 mb-3" size={20} />
            <p className="text-2xl font-black text-white">{totalExercises}</p>
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">Egzersiz</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-900/95 to-slate-950 border border-white/10 rounded-[28px] p-5 shadow-sm">
          ...
          <div className="relative flex items-center gap-3 mb-4">
            <div className="w-12 h-12 rounded-2xl bg-blue-500/10 border border-blue-500/30 flex items-center justify-center text-blue-400"><TrendingUp size={22} /></div>
            <div>
              <p className="text-white text-lg font-black">Gelişim Alanı</p>
              <p className="text-slate-500 text-xs font-semibold">Grafikler ve ölçümler sonraki iterasyonda genişletilecek.</p>
            </div>
          </div>
          <div className="relative grid grid-cols-7 gap-2 mt-5">
            {Array.from({ length: 28 }).map((_, index) => <div key={index} className={`aspect-square rounded-lg ${index % 6 === 0 ? 'bg-blue-500/80 shadow-[0_0_18px_rgba(37,99,235,0.25)]' : 'bg-slate-800/70 border border-white/5'}`} />)}
          </div>
        </div>
      </div>
    </div>
