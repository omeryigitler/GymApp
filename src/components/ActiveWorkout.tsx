...
export function ActiveWorkout({ routine, onFinish, onCancel }: { routine?: Routine; onFinish: (w: Workout) => void; onCancel: () => void }) {
  const { addWorkout, lang } = useAppContext();
  ...
  return (
    <div className="flex flex-col h-full bg-[radial-gradient(circle_at_top,_rgba(37,99,235,0.18),_transparent_36%),#020617] relative">
      ...
      <div className="sticky top-0 z-10 bg-slate-950/80 p-5 flex items-center justify-between border-b border-white/5 backdrop-blur-2xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">{completedSets} set tamamlandı</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-28 hide-scrollbar">
        <input type="text" value={activeWorkout.name} onChange={event => setActiveWorkout({ ...activeWorkout, name: event.target.value })} className="bg-transparent border-none text-[32px] font-black text-white w-full focus:outline-none placeholder-slate-700 my-6 tracking-tight" placeholder={t.workoutName} />
            ...
            <div key={exercise.id} className="bg-gradient-to-br from-slate-900/95 to-slate-950 border border-white/10 rounded-[28px] p-4 shadow-[0_20px_70px_-55px_rgba(37,99,235,0.8)]">
              ...
              <div className="space-y-2">
                {exercise.sets.map((set, setIndex) => (
                  <div key={set.id} className={cn('grid grid-cols-[30px_1fr_1fr_42px] gap-2 items-center rounded-2xl p-1.5 border transition-all', set.completed ? 'bg-blue-500/10 border-blue-500/20' : 'bg-slate-950/35 border-white/5')}>
