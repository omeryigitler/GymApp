...
import { CheckCircle2, Dumbbell, Link2, Plus, Save, Search, Trash2, X } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-[radial-gradient(circle_at_top,_rgba(37,99,235,0.18),_transparent_36%),#020617] relative">
      <div className="sticky top-0 z-10 bg-slate-950/90 p-5 flex items-center justify-between border-b border-slate-900/50 backdrop-blur-xl">
        ...
        <button type="button" onClick={onClose} className="p-2 text-slate-400 hover:text-white bg-slate-900 rounded-full transition-colors"><X size={20} /></button>
      </div>

      <div className="flex-1 overflow-y-auto px-5 pb-36 hide-scrollbar">
        <input type="text" value={name} onChange={event => { setName(event.target.value); setError(''); }} placeholder={t.routineName} className="bg-transparent border-none text-[32px] font-black text-white w-full focus:outline-none placeholder-slate-700 mt-6 mb-5 tracking-tight" />
        ...
        <div className="relative overflow-hidden rounded-[28px] border border-blue-500/40 bg-gradient-to-br from-blue-950/45 via-slate-900/90 to-slate-950 p-5 mb-6 shadow-[0_20px_70px_-35px_rgba(37,99,235,0.9)]">
          <div className="pointer-events-none absolute -top-16 -right-10 w-44 h-44 bg-blue-500/20 rounded-full blur-3xl" />
          <div className="relative flex items-center gap-4 mb-5">
            ...
            <div><h3 className="text-white text-2xl font-black tracking-tight">Egzersiz Ekle</h3><p className="text-slate-400 text-sm font-medium mt-1">{filteredExercises.length} sonuç • toplam {EXERCISES.length} egzersiz</p></div>
          </div>

          <div className="relative mb-4">
            <Search size={19} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none" />
          ...
          <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1 mb-5">
            {muscleFilters.map(filter => {
              ...
              const count = filter === 'all' ? EXERCISES.length : exerciseCounts[filter] || 0;
              return <button type="button" key={filter} onClick={() => setSelectedMuscle(filter)} className={isActive ? 'shrink-0 px-4 py-2 rounded-full text-xs font-extrabold bg-blue-600 text-white shadow-lg shadow-blue-600/25' : 'shrink-0 px-4 py-2 rounded-full text-xs font-extrabold bg-slate-950/60 text-slate-400 border border-white/10 hover:text-white hover:border-blue-500/40'}>{filter === 'all' ? 'Tümü' : filter} <span className="opacity-70">{count}</span></button>;
            })}
          </div>

          <div className="relative space-y-3 max-h-96 overflow-y-auto hide-scrollbar pr-1">
            {filteredExercises.map(exercise => {
              const muscle = exercise.muscle[lang];
              const isJustAdded = lastAddedId === exercise.id;
              return (
                <button type="button" key={exercise.id} onClick={() => addExercise(exercise)} className="w-full group bg-slate-950/45 hover:bg-slate-900/95 border border-white/10 hover:border-blue-500/50 rounded-3xl p-3.5 flex items-center gap-3 text-left transition-all active:scale-[0.99]">
                  ...
                  <div className={isJustAdded ? 'w-11 h-11 rounded-2xl bg-emerald-600 text-white flex items-center justify-center shrink-0 transition-all' : 'w-11 h-11 rounded-2xl border border-blue-500/50 bg-blue-500/10 text-blue-400 flex items-center justify-center shrink-0 group-hover:bg-blue-600 group-hover:text-white transition-all'}>{isJustAdded ? <CheckCircle2 size={22} /> : <Plus size={22} />}</div>
                </button>
              );
            })}
            {filteredExercises.length === 0 && <div className="rounded-3xl border border-dashed border-white/10 p-8 text-center text-slate-500 text-sm font-semibold">Aramaya uygun egzersiz bulunamadı.</div>}
          </div>
        </div>

        <div className="space-y-4 mb-4">
          <div className="flex items-center justify-between pl-2">
            <h3 className="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">Rutindeki Egzersizler</h3>
            <span className="text-[10px] font-black text-blue-300 bg-blue-500/10 border border-blue-500/20 rounded-full px-2.5 py-1">{exercises.length} seçildi</span>
          </div>
          {exercises.length === 0 ? (
            ...
            return (
              <div key={item.id} className="bg-slate-900/90 border border-slate-800 p-5 rounded-3xl shadow-sm">
                <div className="flex justify-between gap-4 mb-5">
                  ...
                  <button type="button" onClick={() => removeExercise(index)} className="text-slate-500 hover:text-red-400 w-9 h-9 rounded-xl hover:bg-red-500/10 flex items-center justify-center shrink-0"><Trash2 size={17} /></button>
                </div>
                ...
                <div className="grid grid-cols-[1fr_auto] gap-2">
                  ...
                  <button type="button" disabled={index === 0} onClick={() => toggleSupersetWithPrevious(index)} className={item.supersetId ? 'px-3 rounded-2xl bg-violet-500/15 border border-violet-500/30 text-violet-300 font-black text-xs' : 'px-3 rounded-2xl bg-slate-950/50 border border-white/10 text-slate-500 font-black text-xs disabled:opacity-40'}><Link2 size={16} className="mx-auto mb-1" />Süper</button>
                </div>
              </div>
            );
          })}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-5 bg-slate-950/95 backdrop-blur-xl max-w-md mx-auto border-x border-slate-900/50">
        ...
        <button type="button" onClick={handleSave} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-extrabold py-4 rounded-2xl transition-all shadow-lg shadow-blue-600/20 active:scale-[0.98] flex items-center justify-center gap-2"><Save size={20} /> {t.save}</button>
      </div>
    </div>
  );
}
